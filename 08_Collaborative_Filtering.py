...
class DotProduct2(Module):
  ...
  def forward(self, batch):
    ...
    dotp = (users * movies).sum(dim=1, keepdim=True) + \
...
# %% how to concatenate 2d tensors
a1 = tensor([1, 3, 3, 7])
b1 = tensor([51, 42, 199, -5, 0, 0, 3])
help(torch.cat)
torch.cat((a1, b1))
# g = torch.Generator()
# g.manual_seed(42)
a2 = torch.zeros(5, 3)
b2 = torch.ones(5, 3)
print(f"cat using default `dim`: {torch.cat((a2, b2))}")
print(f"cat using dim=0: {torch.cat((a2, b2), dim=0)}")
print(f"cat using dim=1: {torch.cat((a2, b2), dim=1)}")
...
class CollabNN(Module):
  def __init__(self, user_sz, item_sz, y_range=(0, 5.5), n_act=100):
    ...
    self.movie_factors = Embedding(*item_sz)
    self.y_range = y_range
  ...
  def forward(self, batch):
    users = self.user_factors(batch[:, 0])
    movies = self.movie_factors(batch[:, 1])
    input = torch.cat((users, movies), dim=1)
    activations = self.layers(input)
    return sigmoid_range(activations, *self.y_range)

embeddings = get_emb_sz(dls)
embeddings
model = CollabNN(*embeddings)
learn = Learner(dls, model, loss_func=MSELossFlat())
learn.fit_one_cycle(5, 5e-3, wd=0.01)

#%% Use `collab_learner` from fastai library
